...
//hashage du mdp pour insérer dans la bd
$userPSWD = password_hash($_SESSION['password'], PASSWORD_DEFAULT);

//insertion nouveau utilisateur dans la base de données
try {
    $request = $bdd->prepare('insert into users (username, password, email, nom, prenom,adresse,  cp) values (:username, :password, :email, :nom, :prenom,:adresse, :cp)');
